...
let rec contains liste e =
  match liste with
    | [] -> false
    | (var, _)::q -> if ((compare e var) == 0) then
                true
              else
                (contains q e)
...
(* Partir d'un environnement (Gamma dans les équ), et d'une expr e = Prod(e1, e2, ..., en) à typer
  ...
  let rec resol_equ list_equ =
    let rec substitution alpha rho list_equ =
      match list_equ with
        | ((t1, t2)::q) ->  if t1 = alpha then (rho, t2)::(substitution alpha rho q) else
                            if t2 = alpha then (t2, rho)::(substitution alpha rho q) else
                            if (t1, t2) = (alpha, alpha) then (rho, rho)::(substitution alpha rho q) else
                            (t1, t2)::(substitution alpha rho q)
        | [] -> []
    in match list_equ with
        | (t::q) -> (match t with
                      | (TInt, TInt) -> resol_equ q
                      | (TBool, TBool) -> resol_equ q
                      | (TUnit, TUnit) -> resol_equ q
                      | (TList(t1), TList(t2)) -> resol_equ ((t1, t2)::q)
                      | (TFun(t1, t2), TFun(sig1, sig2)) -> resol_equ ((t1, t2)::(sig1, sig2)::q)
                      | (TProd(t1, t2), TProd(sig1, sig2)) -> resol_equ ((t1, sig1)::(t2, sig2)::q)
                      | (TVar(alpha1), TVar(alpha2)) -> if (TypeVariable.equal alpha1 alpha2) then resol_equ q else false(* On veut que ce soit les deux mêmes alphas *)
                      | (TVar(alpha), rho) -> resol_equ (substitution (TVar(alpha)) rho q) (* chaud *) (* en fait, ce alpha devient rho dans la suite des équations *)
                      | (rho, TVar(alpha)) -> resol_equ ((TVar(alpha), rho)::q) (* appelle celle d'au dessus en fait *)
                      | _ -> false)
        | [] -> true


let expr = (EApply(EApply(EBinop(PLUS), EConstant(CEntier(2))), EConstant(CEntier(1)))) (* doit renvoier true *)
let expr2 = (EApply(EApply(EBinop(CONS), EConstant(CEntier(2))), EConstant(CNil))) (* doit renvoier true*)
let expr3 = (EApply(EApply(EBinop(PLUS), EConstant(CEntier(2))), EConstant(CBooleen(true)))) (* doit renvoier false *)
let expr4 = (EIf(EConstant(CBooleen(true)), EConstant(CBooleen(true)), EConstant(CEntier(5)))) (* doit renvoier faux *)
let%test _ = let (t, l) = type_unique expr in (resol_equ l)